...
// Patch use the default patch type(Strategic Merge Patch) to patch networkpolicy.
...
func (h *Handler) Patch(original *networkingv1.NetworkPolicy, patch interface{}, patchOptions ...types.PatchType) (*networkingv1.NetworkPolicy, error) {
	...
	case map[string]interface{}:
		modified := &networkingv1.NetworkPolicy{}
...
// strategicMergePatch use the "Strategic Merge Patch" patch type to patch networkpolicy.
//
...
// For further more Strategic Merge patch, see:
//     https://kubernetes.io/docs/tasks/manage-kubernetes-objects/update-api-object-kubectl-patch/#before-you-begin
func (h *Handler) strategicMergePatch(original *networkingv1.NetworkPolicy, patchData []byte) (*networkingv1.NetworkPolicy, error) {
...
// jsonMergePatch use the "JSON Merge Patch" patch type to patch networkpolicy.
...
func (h *Handler) jsonMergePatch(original *networkingv1.NetworkPolicy, patchData []byte) (*networkingv1.NetworkPolicy, error) {
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
	}
...
// diffMergePatch will tak the difference data between original and modified networkpolicy object,
// and use the default patch type(Strategic Merge Patch) patch the differen networkpolicy.
// You can set patchOptions to MergePatchType to use the "JSON Merge Patch" to
// patch networkpolicy.
func (h *Handler) diffMergePatch(original, modified *networkingv1.NetworkPolicy, patchOptions ...types.PatchType) (*networkingv1.NetworkPolicy, error) {
	var (
		err          error
		originalJson []byte
		modifiedJson []byte
	...
	if patchData, err = strategicpatch.CreateTwoWayMergePatch(originalJson, modifiedJson, networkingv1.NetworkPolicy{}); err != nil {
		return nil, err
	}
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
