...
func (h *Handler) WatchByName(name string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	...
	return h.watchNetworkPolicy(listOptions, addFunc, modifyFunc, deleteFunc)
}

// WatchByLabel watch a single or multiple NetworkPolicy resources selected by the label.
...
func (h *Handler) WatchByLabel(labels string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	return h.watchNetworkPolicy(metav1.ListOptions{LabelSelector: labels, TimeoutSeconds: new(int64)},
...
// watchNetworkPolicy watch networkpolicy resources according to listOptions.
func (h *Handler) watchNetworkPolicy(listOptions metav1.ListOptions,
	...
	for {
		...
		// notified of the networkpolicy existence and current state.
