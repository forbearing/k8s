...
// Watch watch all ingressclass resources.
//
...
// WatchByName watch a single ingressclass reseource.
//
// Object as the parameter of addFunc, modifyFunc, deleteFunc:
//  * If Event.Type is Added or Modified: the new state of the object.
//  * If Event.Type is Deleted: the state of the object immediately before deletion.
...
func (h *Handler) WatchByName(name string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	...
	return h.watchIngressClass(listOptions, addFunc, modifyFunc, deleteFunc)
}

// WatchByLabel watch a single or multiple IngressClass resources selected by the label.
...
func (h *Handler) WatchByLabel(labels string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	return h.watchIngressClass(metav1.ListOptions{LabelSelector: labels, TimeoutSeconds: new(int64)},
		addFunc, modifyFunc, deleteFunc)
}

// WatchByField watch a single or multiple IngressClass resources selected by the field.
//
// Object as the parameter of addFunc, modifyFunc, deleteFunc:
//  * If Event.Type is Added or Modified: the new state of the object.
//  * If Event.Type is Deleted: the state of the object immediately before deletion.
...
// watchIngressClass watch ingressclass resources according to listOptions.
func (h *Handler) watchIngressClass(listOptions metav1.ListOptions,
	...
	for {
		...
		// notified of the ingressclass existence and current state.
