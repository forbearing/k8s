...
// Watch watch all storageclass resources.
//
...
// WatchByName watch a single storageclass reseource.
//
// Object as the parameter of addFunc, modifyFunc, deleteFunc:
//  * If Event.Type is Added or Modified: the new state of the object.
//  * If Event.Type is Deleted: the state of the object immediately before deletion.
...
func (h *Handler) WatchByName(name string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	...
	return h.watchStorageClass(listOptions, addFunc, modifyFunc, deleteFunc)
}

// WatchByLabel watch a single or multiple StorageClass resources selected by the label.
...
func (h *Handler) WatchByLabel(labels string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	return h.watchStorageClass(metav1.ListOptions{LabelSelector: labels, TimeoutSeconds: new(int64)},
		addFunc, modifyFunc, deleteFunc)
}

// WatchByField watch a single or multiple StorageClass resources selected by the field.
//
// Object as the parameter of addFunc, modifyFunc, deleteFunc:
//  * If Event.Type is Added or Modified: the new state of the object.
//  * If Event.Type is Deleted: the state of the object immediately before deletion.
...
// watchStorageClass watch storageclass resources according to listOptions.
func (h *Handler) watchStorageClass(listOptions metav1.ListOptions,
	...
	for {
		...
		// notified of the storageclass existence and current state.
