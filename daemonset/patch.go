...
// Patch use the default patch type(Strategic Merge Patch) to patch daemonset.
...
func (h *Handler) Patch(original *appsv1.DaemonSet, patch interface{}, patchOptions ...types.PatchType) (*appsv1.DaemonSet, error) {
	...
	case runtime.Object:
		modified, ok := patch.(*appsv1.DaemonSet)
		if !ok {
			return nil, errors.New("patch data type is not *appsv1.DaemonSet")
		}
		return h.diffMergePatch(original, modified, patchOptions...)
...
// strategicMergePatch use the "Strategic Merge Patch" patch type to patch daemonset.
//
...
// For further more Strategic Merge patch, see:
//     https://kubernetes.io/docs/tasks/manage-kubernetes-objects/update-api-object-kubectl-patch/#before-you-begin
func (h *Handler) strategicMergePatch(original *appsv1.DaemonSet, patchData []byte) (*appsv1.DaemonSet, error) {
	...
	return h.clientset.AppsV1().DaemonSets(namespace).
		Patch(h.ctx, original.Name, types.StrategicMergePatchType, patchData, h.Options.PatchOptions)
}

// jsonMergePatch use the "JSON Merge Patch" patch type to patch daemonset.
...
func (h *Handler) jsonMergePatch(original *appsv1.DaemonSet, patchData []byte) (*appsv1.DaemonSet, error) {
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
	}
...
// jsonPatch use "JSON Patch" patch type to patch daemonset.
//
// For a comparison of JSON patch and JSON merge patch, see:
//     https://erosb.github.io/post/json-patch-vs-merge-patch/
...
func (h *Handler) jsonPatch(original *appsv1.DaemonSet, patchData []byte) (*appsv1.DaemonSet, error) {
	var namespace string
	if len(original.Namespace) != 0 {
		namespace = original.Namespace
	} else {
	...
	return h.clientset.AppsV1().DaemonSets(namespace).Patch(h.ctx,
		original.Name, types.JSONPatchType, patchData, h.Options.PatchOptions)
}

// diffMergePatch will tak the difference data between original and modified daemonset object,
// and use the default patch type(Strategic Merge Patch) patch the differen daemonset.
// You can set patchOptions to MergePatchType to use the "JSON Merge Patch" to
// patch daemonset.
func (h *Handler) diffMergePatch(original, modified *appsv1.DaemonSet, patchOptions ...types.PatchType) (*appsv1.DaemonSet, error) {
	var (
		err          error
		originalJson []byte
		modifiedJson []byte
	...
	if patchData, err = strategicpatch.CreateTwoWayMergePatch(originalJson, modifiedJson, appsv1.DaemonSet{}); err != nil {
		return nil, err
	}
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
