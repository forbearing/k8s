...
func (h *Handler) WatchByName(name string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	...
	return h.watchSecret(listOptions, addFunc, modifyFunc, deleteFunc)
}

// WatchByLabel watch a single or multiple Secret resources selected by the label.
...
func (h *Handler) WatchByLabel(labels string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	return h.watchSecret(metav1.ListOptions{LabelSelector: labels, TimeoutSeconds: new(int64)},
...
// watchSecret watch secret resources according to listOptions.
func (h *Handler) watchSecret(listOptions metav1.ListOptions,
	...
	for {
		...
		// notified of the secret existence and current state.
