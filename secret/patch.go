...
// Patch use the default patch type(Strategic Merge Patch) to patch secret.
...
func (h *Handler) Patch(original *corev1.Secret, patch interface{}, patchOptions ...types.PatchType) (*corev1.Secret, error) {
	...
	case runtime.Object:
		modified, ok := patch.(*corev1.Secret)
		if !ok {
			return nil, errors.New("patch data type is not *corev1.Secret")
		}
		return h.diffMergePatch(original, modified, patchOptions...)
...
// strategicMergePatch use the "Strategic Merge Patch" patch type to patch secret.
//
...
// For further more Strategic Merge patch, see:
//     https://kubernetes.io/docs/tasks/manage-kubernetes-objects/update-api-object-kubectl-patch/#before-you-begin
func (h *Handler) strategicMergePatch(original *corev1.Secret, patchData []byte) (*corev1.Secret, error) {
	...
	return h.clientset.CoreV1().Secrets(namespace).
		Patch(h.ctx, original.Name, types.StrategicMergePatchType, patchData, h.Options.PatchOptions)
}

// jsonMergePatch use the "JSON Merge Patch" patch type to patch secret.
...
func (h *Handler) jsonMergePatch(original *corev1.Secret, patchData []byte) (*corev1.Secret, error) {
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
	}
...
// jsonPatch use "JSON Patch" patch type to patch secret.
//
// For a comparison of JSON patch and JSON merge patch, see:
//     https://erosb.github.io/post/json-patch-vs-merge-patch/
...
func (h *Handler) jsonPatch(original *corev1.Secret, patchData []byte) (*corev1.Secret, error) {
	var namespace string
	if len(original.Namespace) != 0 {
		namespace = original.Namespace
	} else {
	...
	return h.clientset.CoreV1().Secrets(namespace).Patch(h.ctx,
		original.Name, types.JSONPatchType, patchData, h.Options.PatchOptions)
}

// diffMergePatch will tak the difference data between original and modified secret object,
// and use the default patch type(Strategic Merge Patch) patch the differen secret.
// You can set patchOptions to MergePatchType to use the "JSON Merge Patch" to
// patch secret.
func (h *Handler) diffMergePatch(original, modified *corev1.Secret, patchOptions ...types.PatchType) (*corev1.Secret, error) {
	var (
		err          error
		originalJson []byte
		modifiedJson []byte
	...
	if patchData, err = strategicpatch.CreateTwoWayMergePatch(originalJson, modifiedJson, corev1.Secret{}); err != nil {
		return nil, err
	}
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
