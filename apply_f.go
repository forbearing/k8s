...
func ApplyF(ctx context.Context, kubeconfig, filename string) error {
	...
	for _, k8sResource := range k8sResourceItems {
		...
		case *corev1.Namespace:
			...
			if ns, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply namespace %q failed: %v\n", ns.Name, err)
			}
		case *corev1.Service:
			...
			if svc, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply service %q failed: %v\n", svc.Name, err)
			}
		case *corev1.ConfigMap:
			...
			if cm, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply configmap %q failed: %v\n", cm.Name, err)
			}
		case *corev1.Secret:
			...
			if sec, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply secret %q failed: %v\n", sec.Name, err)
			}
		case *corev1.ServiceAccount:
			...
			if sa, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply serviceaccount %q failed: %v\n", sa.Name, err)
			}
		case *corev1.Pod:
			...
			if p, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply pod %q failed: %v\n", p.Name, err)
			}
		case *corev1.PersistentVolume:
			...
			if pv, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply persistentvolume %q failed: %v\n", pv.Name, err)
			}
		case *corev1.PersistentVolumeClaim:
			...
			if pvc, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply persistentvolumeclaim %q failed: %v\n", pvc.Name, err)
			}
		case *appsv1.Deployment:
			...
			if deploy, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply deployment %q failed: %v\n", deploy.Name, err)
			}
		case *appsv1.StatefulSet:
			...
			if sts, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply statefulset %q failed: %v\n", sts.Name, err)
			}
		case *appsv1.DaemonSet:
			...
			if ds, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply daemonset %q failed: %v\n", ds.Name, err)
			}
		case *networking.Ingress:
			...
			if ing, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply ingress %q failed: %v\n", ing.Name, err)
			}
		case *networking.IngressClass:
			...
			if ingc, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply ingressclass %q failed: %v\n", ingc.Name, err)
			}
		case *networking.NetworkPolicy:
			...
			if netpol, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply networkpolicy %q failed: %v\n", netpol.Name, err)
			}
		case *batchv1.Job:
			...
			if j, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply job %q failed: %v\n", j.Name, err)
			}
		case *batchv1.CronJob:
			...
			if cj, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply cronjob %q failed: %v\n", cj.Name, err)
			}
		case *rbacv1.Role:
			...
			if r, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply role %q failed: %v\n", r.Name, err)
			}
		case *rbacv1.RoleBinding:
			...
			if rb, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply rolebinding %q failed: %v\n", rb.Name, err)
			}
		case *rbacv1.ClusterRole:
			...
			if cr, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply clusterrole %q failed: %v\n", cr.Name, err)
			}
		case *rbacv1.ClusterRoleBinding:
			...
			if crb, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply clusterrolebinding %q failed: %v\n", crb.Name, err)
			}
		case *corev1.ReplicationController:
			...
			if rc, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply replicationcontroller %q failed: %v\n", rc.Name, err)
			}
		case *appsv1.ReplicaSet:
			...
			if rs, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply replicaset %q failed: %v\n", rs.Name, err)
			}
		case *storagev1.StorageClass:
			...
			if sc, err := handler.Apply(k8sResource); err != nil {
				klog.V(4).Infof("apply storageclass %q failed: %v\n", sc.Name, err)
			}
		default:
		}
	}
