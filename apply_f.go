	...
	for _, k8sResource := range k8sResourceItems {
		...
		case *corev1.Pod:
			...
			if p, err := handler.Apply(k8sResource); err != nil {
				log.Errorf("apply pod %q failed: %v\n", p.Name, err)
			}
		...
		case *networking.Ingress:
			...
			if ing, err := handler.Apply(k8sResource); err != nil {
				log.Errorf("apply ingress %q failed: %v\n", ing.Name, err)
			}
		...
		case *rbacv1.RoleBinding:
			...
			if rb, err := handler.Apply(k8sResource); err != nil {
				log.Errorf("apply rolebinding %q failed: %v\n", rb.Name, err)
			}
		case *rbacv1.ClusterRole:
			...
			if cr, err := handler.Apply(k8sResource); err != nil {
				log.Errorf("apply clusterrole %q failed: %v\n", cr.Name, err)
			}
		...
		case *appsv1.ReplicaSet:
			...
			if rs, err := handler.Apply(k8sResource); err != nil {
				log.Errorf("apply replicaset %q failed: %v\n", rs.Name, err)
			}
		case *storagev1.StorageClass:
			...
			if sc, err := handler.Apply(k8sResource); err != nil {
				log.Errorf("apply storageclass %q failed: %v\n", sc.Name, err)
			}
		default:
		}
	}
