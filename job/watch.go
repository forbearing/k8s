...
func (h *Handler) WatchByName(name string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	...
	return h.watchJob(listOptions, addFunc, modifyFunc, deleteFunc)
}

// WatchByLabel watch a single or multiple Job resources selected by the label.
...
func (h *Handler) WatchByLabel(labels string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	return h.watchJob(metav1.ListOptions{LabelSelector: labels, TimeoutSeconds: new(int64)},
...
// watchJob watch job resources according to listOptions.
func (h *Handler) watchJob(listOptions metav1.ListOptions,
	...
	for {
		...
		// notified of the job existence and current state.
