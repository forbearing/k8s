...
// Patch use the default patch type(Strategic Merge Patch) to patch persistentvolumeclaim.
...
func (h *Handler) Patch(original *corev1.PersistentVolumeClaim, patch interface{}, patchOptions ...types.PatchType) (*corev1.PersistentVolumeClaim, error) {
	...
	case map[string]interface{}:
		modified := &corev1.PersistentVolumeClaim{}
	...
	case runtime.Object:
		modified, ok := patch.(*corev1.PersistentVolumeClaim)
		if !ok {
			return nil, errors.New("patch data type is not *corev1.PersistentVolumeClaim")
		}
		return h.diffMergePatch(original, modified, patchOptions...)
...
// strategicMergePatch use the "Strategic Merge Patch" patch type to patch persistentvolumeclaim.
//
...
// For further more Strategic Merge patch, see:
//     https://kubernetes.io/docs/tasks/manage-kubernetes-objects/update-api-object-kubectl-patch/#before-you-begin
func (h *Handler) strategicMergePatch(original *corev1.PersistentVolumeClaim, patchData []byte) (*corev1.PersistentVolumeClaim, error) {
...
// jsonMergePatch use the "JSON Merge Patch" patch type to patch persistentvolumeclaim.
...
func (h *Handler) jsonMergePatch(original *corev1.PersistentVolumeClaim, patchData []byte) (*corev1.PersistentVolumeClaim, error) {
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
	}
...
func (h *Handler) jsonPatch(original *corev1.PersistentVolumeClaim, patchData []byte) (*corev1.PersistentVolumeClaim, error) {
	var namespace string
	if len(original.Namespace) != 0 {
		namespace = original.Namespace
	} else {
	...
	return h.clientset.CoreV1().PersistentVolumeClaims(namespace).Patch(h.ctx,
		original.Name, types.JSONPatchType, patchData, h.Options.PatchOptions)
}

// diffMergePatch will tak the difference data between original and modified persistentvolumeclaim object,
// and use the default patch type(Strategic Merge Patch) patch the differen persistentvolumeclaim.
// You can set patchOptions to MergePatchType to use the "JSON Merge Patch" to
// patch persistentvolumeclaim.
func (h *Handler) diffMergePatch(original, modified *corev1.PersistentVolumeClaim, patchOptions ...types.PatchType) (*corev1.PersistentVolumeClaim, error) {
	var (
		err          error
		originalJson []byte
		modifiedJson []byte
	...
	if patchData, err = strategicpatch.CreateTwoWayMergePatch(originalJson, modifiedJson, corev1.PersistentVolumeClaim{}); err != nil {
		return nil, err
	}
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
