...
func (h *Handler) WatchByName(name string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	...
	return h.watchService(listOptions, addFunc, modifyFunc, deleteFunc)
}

// WatchByLabel watch a single or multiple Service resources selected by the label.
...
func (h *Handler) WatchByLabel(labels string, addFunc, modifyFunc, deleteFunc func(obj interface{})) error {
	return h.watchService(metav1.ListOptions{LabelSelector: labels, TimeoutSeconds: new(int64)},
...
// watchService watch service resources according to listOptions.
func (h *Handler) watchService(listOptions metav1.ListOptions,
	...
	for {
		...
		// notified of the service existence and current state.
