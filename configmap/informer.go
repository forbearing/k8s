...
// ConfigMapInformer returns underlying ConfigMapInformer which provides
// access to a shared informer and lister for configmap.
func (h *Handler) ConfigMapInformer() informerscore.ConfigMapInformer {
	return h.informerFactory.Core().V1().ConfigMaps()
}
...
func (h *Handler) Informer() cache.SharedIndexInformer {
	return h.informerFactory.Core().V1().ConfigMaps().Informer()
}

// Lister returns underlying ConfigMapLister which helps list configmaps.
func (h *Handler) Lister() listerscore.ConfigMapLister {
	return h.informerFactory.Core().V1().ConfigMaps().Lister()
}
...
// and delete event of k8s configmap resource, respectively.
