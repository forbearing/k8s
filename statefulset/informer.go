...
// StatefulSetInformer returns underlying StatefulSetInformer which provides
// access to a shared informer and lister for statefulset.
func (h *Handler) StatefulSetInformer() informersapps.StatefulSetInformer {
	return h.informerFactory.Apps().V1().StatefulSets()
}
...
func (h *Handler) Informer() cache.SharedIndexInformer {
	return h.informerFactory.Apps().V1().StatefulSets().Informer()
}
...
func (h *Handler) Lister() listersapps.StatefulSetLister {
	return h.informerFactory.Apps().V1().StatefulSets().Lister()
}
...
// and delete event of k8s statefulset resource, respectively.
