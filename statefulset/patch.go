...
// Patch use the default patch type(Strategic Merge Patch) to patch statefulset.
...
func (h *Handler) Patch(original *appsv1.StatefulSet, patch interface{}, patchOptions ...types.PatchType) (*appsv1.StatefulSet, error) {
	...
	case metav1.Object, runtime.Object:
		modified, ok := patch.(*appsv1.StatefulSet)
		if !ok {
			return nil, errors.New("patch data type is not *appsv1.StatefulSet")
		}
		return h.diffMergePatch(original, modified, patchOptions...)
...
// strategicMergePatch use the "Strategic Merge Patch" patch type to patch statefulset.
//
...
// For further more Strategic Merge patch, see:
//     https://kubernetes.io/docs/tasks/manage-kubernetes-objects/update-api-object-kubectl-patch/#before-you-begin
func (h *Handler) strategicMergePatch(original *appsv1.StatefulSet, patchData []byte) (*appsv1.StatefulSet, error) {
...
// jsonMergePatch use the "JSON Merge Patch" patch type to patch statefulset.
...
func (h *Handler) jsonMergePatch(original *appsv1.StatefulSet, patchData []byte) (*appsv1.StatefulSet, error) {
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
	}
...
func (h *Handler) jsonPatch(original *appsv1.StatefulSet, patchData []byte) (*appsv1.StatefulSet, error) {
	namespace := original.GetNamespace()
	if len(namespace) == 0 {
		namespace = h.namespace
	}
	return h.clientset.AppsV1().StatefulSets(namespace).Patch(h.ctx,
		original.Name, types.JSONPatchType, patchData, h.Options.PatchOptions)
}

// diffMergePatch will tak the difference data between original and modified statefulset object,
// and use the default patch type(Strategic Merge Patch) patch the differen statefulset.
// You can set patchOptions to MergePatchType to use the "JSON Merge Patch" to
// patch statefulset.
func (h *Handler) diffMergePatch(original, modified *appsv1.StatefulSet, patchOptions ...types.PatchType) (*appsv1.StatefulSet, error) {
	var (
		err          error
		originalJson []byte
		modifiedJson []byte
	...
	if patchData, err = strategicpatch.CreateTwoWayMergePatch(originalJson, modifiedJson, appsv1.StatefulSet{}); err != nil {
		return nil, err
	}
	if len(patchData) == 0 || string(patchData) == "{}" {
		return original, nil
